...
from mysolution import ShortestPath
...
filepath = ("master_database_updated.csv") # This is where we save the running state-action pairs and their values across all episodes
column_names = ['State', 'Action', 'Count', 'Value'] #Names of the columns within the master db

# The two lines below decide whether or not we are starting a new database or reading in an old one. Currently we are reading in a database of
# state-action pairs that were generated by the random agent
...
iterations = 10 #This sets the number of episodes to run
...
MySolution.updateReference(my_solution, master_df) #Update the policy function's database of state action pairs by passing it the current master dataframe
...
for i in range(iterations):
    ...
    #After each episode, per the Direct Evaluation Algorithm, we need to update the values of the state-action pairs

    # During the episode, each state-action pair is assigned a "value" of "TBD". This is because the score comes after the episode finishes.
    # All we're doing here is backtracking a swapping the placeholder out for the episode score
    condition = my_solution.df['Value'] == "TBD" 
    my_solution.df.loc[condition, 'Value'] = -1 * metrics.score



    # Assuming 'State' and 'Action' are the columns in both dataframes, set 'State' and 'Action' as the columns to match on
    columns_to_match = ['State', 'Action']

    # Iterate through the current database 
    for index, row in my_solution.df.iterrows(): #Iterate through every row in our episode's database

        # Extract the values from the current row
        values_to_check = [row[column] for column in columns_to_match] #This gets the state and action of a row
        new_value = row['Value'] #The value from the episode for that state-action pair
        ...
        if match.any():
            ...
            master_df.loc[match, 'Value'] = ((master_df.loc[match, 'Value'] * master_df.loc[match, 'Count'])  + new_value) / (master_df.loc[match, 'Count'] + 1) #Calculate the new average after adding in the new score
            master_df.loc[match, 'Count'] += 1 #incement the number of times this state-action pair has been seen
    ...
    MySolution.updateReference(my_solution, master_df) #Update the policy functions database of state action pairs by passing it the updated master dataframe
    ...
    print("Score:             {}".format(metrics.score) + "EPISODE NUMBER:" + str(i)) #prints out the score for the episode that just occured




# Save the updated master database to a file for later use
