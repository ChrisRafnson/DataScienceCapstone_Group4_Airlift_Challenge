...
class ShortestPath(Solution):
    ...
    def policies(self, obs, dones, infos=0):
        ...
        #RECORDING CODE FOR MC ALGO
        gs = self.get_state(obs)

        agent = gs["agents"]["a_0"] #grab the agent, there is only one currently

        self.MC.current_reduced_state = [agent["state"], #current agent state
        ...
        reduced_state_string = str(self.MC.current_reduced_state)
        action = str(actions)
        buffer_entry = [reduced_state_string, action, 0]
        self.MC.episode_buffer.append(buffer_entry)
