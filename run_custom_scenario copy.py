...
filepath = ("master_df.csv") # This is where we save the running state-action pairs and their values across all episodes
filepath2 = ("episode_results.csv") #This is a record of episode results
column_names = ['State', 'Action', 'Episode', 'Value'] #Names of the columns within the master db
...
episode_record = pd.DataFrame(columns = column_names) 
 
#Initial value for episode number
MySolution.episode_num = 1

iterations = 5 #This sets the number of episodes to run
...
for i in range(iterations):
    ...
    condition = my_solution.df['Episode'] == "TBD" 
    my_solution.df.loc[condition, 'Episode'] = i

    combined_data = pd.concat([episode_record, my_solution.df], axis=0, ignore_index=True)
...
combined_data.to_csv(filepath2, index=False)
